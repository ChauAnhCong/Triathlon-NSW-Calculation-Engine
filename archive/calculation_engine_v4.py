...
#         (season_source['League Name'] == round_info['league']) & 
#         (season_source['Round'] == round_info['round'])
#     ].iloc[0]
    
#     # Get allowed clubs for this league
#     allowed_clubs = [c.strip() for c in league_info['Clubs'].split(',')]
    
#     # Read all sheets from round file
#     xl = pd.ExcelFile(round_info['path'])
#     all_results = []
    
#     for sheet_name in xl.sheet_names:
#         # Skip non-race sheets
#         if 'summary' in sheet_name.lower() or 'points' in sheet_name.lower():
#             continue
...
#         # Validate clubs
#         invalid_clubs = race_df[
#             ~race_df['Club Name'].apply(lambda x: validate_club(x, allowed_clubs))
#         ]['Club Name'].unique()
        
#         if len(invalid_clubs) > 0:
#             print(f"Warning: Invalid clubs found in {sheet_name}: {invalid_clubs}")
            
#         # Add race metadata
#         race_df['Race'] = sheet_name
#         race_df['Performance_Eligible'] = race_validation['performance_eligible']
#         race_df['Double_Points'] = race_validation['double_points']
...
#     # Calculate points considering eligibility and double points
#     points_df = calculate_participation_points(
#         results_df[results_df['Performance_Eligible']],  # Only performance eligible races
#         league_df
#     )
    
#     if any(results_df['Double_Points']):
...
#     output_filename = f"Round_{round_info['round']}_{round_info['league']}_{datetime.now().strftime('%Y%m%d')}.xlsx"
#     output_path = os.path.join(OUTPUT_DIR, output_filename)
    
#     # Save round results
#     with pd.ExcelWriter(output_path) as writer:
#         points_df.to_excel(writer, sheet_name='League Points', index=False)
#         mvp_df.to_excel(writer, sheet_name='Round MVPs', index=False)
    
#     # Update season history
#     update_season_history(results_df, round_info)
...
#     return results_df.groupby('Club Name')['Performance Points'].sum().to_dict()
...
#         league_df: DataFrame with league information
#         race_validation: Dict containing race eligibility info
#     """
#     # Handle race validation
...
#     numeric_cols = ['45 PTS (20%)', '30 PTS (10%)', '15PTS (5%)']
...
#     # Calculate performance points only if race is eligible
...
#     # Get season source of truth
#     season_source_path = get_season_source_of_truth()
#     if not season_source_path:
#         print("Error: No season source file found")
#         return
        
#     season_source = pd.read_excel(season_source_path)
    
#     # Find new round files to process
#     round_files = find_new_round_files()
#     if not round_files:
#         print("No new round files to process")
#         return
...
#     for round_info in round_files:
#         print(f"Processing {round_info['filename']}...")
        
#         # Get league info for this round
#         league_info = season_source[
#             (season_source['League Name'] == round_info['league']) & 
#             (season_source['Round'] == round_info['round'])
#         ].iloc[0]
        
#         # Process each sheet in the round file
#         xl = pd.ExcelFile(round_info['path'])
#         all_results = []
...
#             race_validation = validate_race_type(sheet_name, league_info)
#             if not (race_validation['performance_eligible'] or race_validation['participation_eligible']):
#                 print(f"Skipping {sheet_name} - not a valid race type")
#                 continue
            
#             # Read and process race sheet
#             race_df = xl.parse(sheet_name)
#             league_df = xl.parse('League one', header=1)
            
#             # Clean up dataframes
#             for df in [race_df, league_df]:
#                 df.columns = [col.strip() for col in df.columns]
#                 df.dropna(subset=['Triathlon Club' if 'Triathlon Club' in df.columns else 'Club'], inplace=True)
            
#             # Calculate points for this race
#             points_df = calculate_participation_points(race_df, league_df, race_validation)
#             mvp_df = calculate_individual_points(race_df)
            
#             # Save results to output file
#             output_filename = f"{round_info['league']}_R{round_info['round']}_{sheet_name}_{datetime.now().strftime('%Y%m%d')}.xlsx"
#             output_path = os.path.join(OUTPUT_DIR, output_filename)
            
#             with pd.ExcelWriter(output_path) as writer:
#                 columns_order = ['Club', 'ICL Eligible Number', '45 PTS (20%)', '30 PTS (10%)', 
#                                '15PTS (5%)', 'Finishers', 'Participation Points', 
#                                'Performance Points', 'Total Points']
#                 points_df[columns_order].to_excel(writer, sheet_name='League Points', index=False)
#                 mvp_df.to_excel(writer, sheet_name='Club MVPs', index=False)
#                 race_df.to_excel(writer, sheet_name='Results', index=False)
        
#         # Move processed file to archive
#         shutil.move(round_info['path'], 
#                    os.path.join(PROCESSED_DIR, round_info['filename']))
        
#     # Generate season reports after processing all rounds
#     # generate_season_reports()
