...
def calculate_performance_points(results_df):
    ...
    # DEBUG: Show individual performance points
    ...
    # CRITICAL: Return as dictionary for proper mapping
...
def calculate_participation_points(results_df, icl_df, race_validation):
    ...
    # CRITICAL FIX: Performance points calculation and storage
    if race_validation['performance_eligible']:
        # Calculate performance points (this part was working)
        performance_points = calculate_performance_points(results_df)
        
        # BUG WAS HERE: The performance_points dict wasn't being properly mapped to icl_df
        # FIX: Properly map the calculated performance points to the clubs
        icl_df['Performance Points'] = icl_df['Club'].map(performance_points).fillna(0)
        
        # Apply double points if specified
        if race_validation['double_points']:
            icl_df['Performance Points'] *= 2
    else:
    ...
    print(f"DEBUG - Returning race points:")
    print(icl_df[['Club', 'Participation Points', 'Performance Points', 'Total Points']])
...
def generate_round_summary(all_points_dfs):
    """Generate round summary with participation and performance breakdowns"""
    
    print("\n=== ROUND SUMMARY CALCULATION DEBUG ===")
    
    # STEP 1: Show individual race results for debugging
    for i, race_points_df in enumerate(all_points_dfs):
        print(f"\nRace {i+1} points contribution:")
        print(race_points_df[['Club', 'Participation Points', 'Performance Points', 'Total Points']])
    
    # STEP 2: Combine all race points
    combined_points = pd.concat(all_points_dfs, ignore_index=True)
    
    print("\nCombined points before aggregation:")
    print(combined_points[['Club', 'Participation Points', 'Performance Points', 'Total Points']])
    
    # STEP 3: Sum across all races within the round by club
    round_summary = combined_points.groupby('Club').agg({
        'Participation Points': 'sum',  # Sum participation across Sprint + Standard + Aquabike
        'Performance Points': 'sum',    # Sum performance across Sprint + Standard (Aquabike = 0)
        'ICL Eligible Number': 'first'  # Take first (should be same for all races)
    }).reset_index()
    
    # STEP 4: Recalculate total points
    ...
    print(round_summary[['Club', 'Participation Points', 'Performance Points', 'Total Points']])
    
    # STEP 5: Sort by total points descending
...
def process_round_file(round_info, season_source):
    ...
    try:
        ...
        for sheet_name in xl.sheet_names:
            ...
            try:
                ...
                if icl_df is not None:
                    ...
                    # CRITICAL DEBUG: Check what calculate_participation_points returns
                    points_df = calculate_participation_points(race_df, icl_df.copy(), race_validation)
                    
                    print(f"Points calculation returned:")
                    ...
                    print(f"Participation Points column values: {points_df['Participation Points'].tolist()}")
                    
                    points_df['Race_Type'] = sheet_name
                    
                    print(f"Before appending to all_points:")
                    print(points_df[['Club', 'Participation Points', 'Performance Points', 'Total Points']])
        ...
        round_summary = generate_round_summary(all_points)
